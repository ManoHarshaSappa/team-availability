// api/availability.js
// Serverless endpoint (Vercel) that reads/writes a JSON file in your GitHub repo.
// Set these env vars in your Vercel project:
//   GITHUB_TOKEN  (fine-grained PAT with Contents: Read/Write for the repo)
//   GITHUB_OWNER  (your GitHub username or org)
//   GITHUB_REPO   (repo name, e.g. "team-availability")
// Optional:
//   GITHUB_FILE_PATH (defaults to "availability.json")
...
export default async function handler(req, res) {
  ...
  const base = `https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${encodeURIComponent(
    GITHUB_FILE_PATH
  )}`;
  ...
  if (req.method === "POST") {
    ...
    if (!put.ok) {
      return res.status(put.status).send(await put.text());
    }
